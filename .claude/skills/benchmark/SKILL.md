...
- `msaa=<0..8>` — **Testbed only**, and the newest of these (#298). How many multisample samples the scene target carries; ignored above `ssaa` 1, where the supersample resolve already averages geometry edges. It exists because **both quality rungs below `High` run at `ssaa` 1 and therefore carry all eight samples**, which nothing had ever charged for. The `[fps]` line prints `msaa Nx (asked M)` — the count the target was **actually created with** beside the one requested, because a driver may clamp a sample count silently and a sweep whose halves are secretly identical is trap 8 in another costume. Measured on the reference desktop at 3840×1600, `ssaa=1`: 8× → none is 1.12 ms on the mountain, 0.61 meadow, 0.58 neon city and 0.30 cavern, and **8× → 4× is free everywhere** (0.04–0.07). The cavern's seventh-sized saving is the #155 signature again — trap 7's scene is the same scene here.
- `rscale=<0.25..1>` — **Testbed only** (#298). Render the scene at this fraction of the back buffer and magnify it back; ignored above `ssaa` 1. The `[fps]` line prints the target's own size beside it, which is what to read back. **It is the only lever that moves the cavern and the dream**: those two shade a backdrop the size of the back buffer and scale it up (#155) — trap 7 — but that path runs only *above* `ssaa` 1, so below native they draw into the smaller target like everything else. Measured on the reference desktop at 3840×1600: 0.85× saves 0.59–1.05 ms and 0.75× saves 0.92–1.63 across mountain/cavern/neon/meadow. It **stacks with `msaa=`**, 88–95 % additively. Its `rscale=1` column reproduced the `msaa=8` column of a separate sweep to within 0.02 ms on all four scenes, which is the cheapest consistency check either sweep has. **⚠ It is an instrument, not a candidate:** the owner ruled on 2026-08-28, with those figures in hand, that the game always renders at the display's native resolution and **no quality tier may ever lower it** — a tier drops effects, not pixels. Sweep it freely; never ship it. What it is genuinely good for is the question this section's last paragraph asks — whether a pass is pixel-bound — **downwards**, where `ssaa` can only ask upwards, and on the cavern and the dream `ssaa` cannot ask at all (trap 7).
...
- `scene=<city|sea|savanna|desert|mountain|meadow|neon|forest|space|dream|cavern|moon|outback|tropical|volcano|mars|storm>`, `sky=<1..20>` — pin the backdrop. The scene names are

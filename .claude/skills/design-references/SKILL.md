...
**Reading it:** klein is three times faster in the same memory and follows wording at least as well, so it is the natural pick for scenes, settings and anything iterated over many seeds; Z-Image still draws the richer applied ornament and the more useful prop sheet, and never stamps a corner mark. A klein corner stamp is a crop, not a lost reference — but check the corners before handing one on. **The owner's verdict (2026-09-26, given while #622's references were rendering): klein draws the better pictures, and every reference set is rendered with BOTH models** — the same prompt file into a second `-Out` folder with the klein flags above, queued after the Z-Image run. Two things make it a queue and not a pair: the two do not fit on the card together, and the script attaches to whatever server already holds port 7860, whichever model it was started with — so a klein run started while the Z-Image server is still up renders Z-Image into the klein folder. Wait for the port to be free (`netstat -an | grep :7860`) between them. The script's default stays Z-Image so an old command line renders what it always did.

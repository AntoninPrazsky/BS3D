...
namespace Prazsky.Core.Render
{
    ...
    /// with distance, the whole surface falling away with the square of the distance so the horizon closes
    /// a few hundred units out the way a small world's horizon does. There is no haze dial and no wind —
    /// there is no air.
    /// </summary>
    public sealed class MoonTerrainConfig
    {
        ...
        /// closes the horizon. 8e-5 is a 6.25 km "moon" — the horizon stands 360–450 units out from the
        /// play and menu cameras' heights, and the skyline is a hard sunlit line against black. This
        /// replaces the horizon haze every atmospheric sibling fades out with: there is no air to scatter,
        /// so distance here is occlusion, not fog. It is also sized against the <b>Game camera's 500-unit
        /// far plane</b>: the horizon must close by occlusion before the far plane can cut the terrain, or
        /// the skyline turns into the clip plane — dead level and camera-locked. Halve it and it does.
        /// </summary>

...
namespace BS3D
{
    ...
    public class BS3DGame : Microsoft.Xna.Framework.Game
    {
        ...
        /// <summary>
        /// Marks the level lost for the stated reason. It does <b>not</b> tear the session down here — a loss can
        /// be reached from the middle of <see cref="Update"/> (a shot that spends the budget, a frame that slides
        /// the ceiling past the line), and rebuilding mid-frame would leave the rest of the frame running against
        /// a simulation that no longer exists. The flag is acted on at the end of <see cref="Update"/>, the way
        /// <see cref="_clearedCountdown"/> is. The result screen of #59 will replace the silent retry with a
        /// screen that states the reason and offers Retry — the lose conditions (#58) were built before that
        /// screen, so the seam they hand off to is this method, and #59 will swap its body.
        /// </summary>
        private void LoseLevel(string reason)
        {
            ...
            //must not stack a second teardown onto the first.
            if (_levelLost) return;
            _levelLost = true;

            Console.WriteLine($"[level] Lost '{LevelName(_levelIndex)}' — {reason}, score {_score.Score}");
        }
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            //Last, because each of these may tear the whole session down and build a new one in its place —
            //anything above them this frame would then be running against a simulation that no longer exists.
            //
            //A loss goes first: a lost level has no clear to honour, and if both were set on the same frame the
            //loss is the one the player's last shot earned. The reload here is the real one BuildLevel does, and
            //for now it lands straight back in play — the result screen of #59 will hold the loss on screen and
            //offer Retry instead.
            if (_levelLost)
            {
                BuildLevel(_levelIndex);

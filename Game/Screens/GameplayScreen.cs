...
namespace BS3D.Screens
{
    ...
    internal sealed partial class GameplayScreen : Screen
    {
        ...
        //Whether the cursor is the aim's or the desktop's (#99, #154). Taken on arrival at play — pressing
        //the menu entry that put this screen on top is the opt-in — and given back on every focus loss,
        //after which it is only re-taken by a click inside the game's own picture: until then the window
        //can be dragged by its title bar, resized, or left alone for another application.
        ...
        public override void CoveredChanged()
        {
            ...
            //cost every level start. #99's two hazards are both still answered: the pressing click cannot
            //fire a shot, because the shot edge sits behind Invalidate's dropped baseline and is measured
            //against the mouse snapshot below, which already knows the button is down (same hazard, and the
            //same fix, as the menu's own _menuClickArmed); and the aim cannot yank, because ApplyCursor
            //applies nothing until the first Recentre has re-based the delta. A mid-play focus loss is not
            //this path — Update's inactive branch frees the cursor, and THAT recovery still takes a click in
            //the picture. An arrival with the window unfocused stays free for the same reason: there is no
            //press to read intent from, and the first frame back would only take the pointer hostage.

...
//near-horizontal, so the particles read as spray torn off the waves rather than a falling veil.
//
...
SprayVertexOutput SprayVS(SprayVertexInput input)
{
	...
	//wisps (the spindrift haze). The wisps stay small - large billboards read as bokeh orbs, not mist.
	float isMist = step(r, 0.4);
	float size = DropletSize * lerp(0.5 + 1.0 * r, 2.5 + 2.5 * r, isMist);
...
{
	//Soft round droplet: opaque at the centre, feathered to nothing at the rim

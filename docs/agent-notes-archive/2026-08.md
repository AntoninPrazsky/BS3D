# Agent notes — archiv 2026-08

Zápisy z **srpna 2026**, přenesené sem beze změny při rotaci deníku (#358). Nic tu není shrnuté ani přepsané: zápisy se citují po měsících zpátky a hodnotu má přesná formulace, ne její výtah.

Aktuální měsíc a pravidla jsou v `docs/agent-notes.md`. **Hledej přes obojí** — `grep -r "..." docs/agent-notes.md docs/agent-notes-archive/`.
